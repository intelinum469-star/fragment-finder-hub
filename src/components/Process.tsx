        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map(i => (
            <div key={i} className="p-6 rounded-3xl shadow-xl bg-white">
              <div className="text-4xl font-black text-[#F5569B] mb-2">{i}</div>
              <h3 className="text-xl font-black mb-2">{t(`step${i}_title`)}</h3>
              <p className="text-sm text-black">{t(`step${i}_text`)}</p>
            </div>
          ))}
        </div>
      </div>
